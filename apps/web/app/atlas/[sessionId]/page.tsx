...
export default async function AtlasPage({ params }: AtlasPageProps) {
  ...
  if (!env.MONGODB_URI || !env.MONGODB_DB || !env.R2_PUBLIC_BASE_URL) {
    return (
      <main className="mx-auto flex min-h-dvh max-w-3xl flex-col items-center justify-center px-4 py-16 text-center">
        <h1 className="text-2xl font-bold">{t.atlasNeedsPersistence}</h1>
        <p className="mt-4 opacity-70">
          {t.atlasPersistenceHelp}
        </p>
        <Link
          href="/play"
  ...
  if (!rows || rows.length === 0) {
    return (
      <main className="mx-auto flex min-h-dvh max-w-3xl flex-col items-center justify-center px-4 py-16 text-center">
        <h1 className="text-2xl font-bold">{t.noSessionPages}</h1>
        <p className="mt-4 opacity-70">
          {t.noSessionPagesHelp}
        </p>
  ...
  return (
    <AtlasView
      ...
      rootTitle={root?.title ?? t.sessionHistory}
